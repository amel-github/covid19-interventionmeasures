...
min_date_100_cases <- rep(0, length(countries_with_measures))
...
for(i in 1:length(countries_with_measures)) {
  ...
  date_cases <- JH_data_one_country$date[JH_data_one_country$cases>=100]
  if(length(date_cases)!=0){
    min_date_100_cases[i] <- min(date_cases)
    if(nrow(JH_data_one_country)>1 & nrow(onecountry_measures)>0){
      for(j in 3:dim(measures)[2]){
        if(sum(onecountry_measures[,j])>0){
          ...
          if(!is.na(min_date) & !is.na(min_date_100_cases[i])){i
            min_date <- as.Date(min_date)
            first_date_matrix[i,(j-2)] <- as.numeric(min_date - min_date_100_cases[i])
          } }
      } } } } 
...
name <- "Measures_Countries_time_of_activation_zeroday100cases.png"
ggsave(plot,  file = name,  width = 33*1.25, height = 27 *1.25, units = "cm", dpi=96) 
name <- "Measures_Countries_time_of_activation_zeroday100cases.svg"
ggsave(plot,  file = name,  width = 33*1.25, height = 27 *1.25, units = "cm", dpi=96) 
